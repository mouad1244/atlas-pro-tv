...
import { PhotoMarquee } from "./marquee";
...
const AVATAR_PHOTOS = [
  "0Ah8NJHMZD0d3RRxeP71fvSIsiU.avif",
  "L6L7vVEDLKOhB3BZEEDfhT1pvNs.avif",
  "RACrj3qncZKB8VjBcz2K0jZad4Y.avif",
  "THZhAwqkSvzgFdFMNLINU4oka0.avif",
].map((name) => `/assets/avatars/${name}`);
  ...
  return (
    <section id="home" className="relative overflow-hidden bg-ink pt-20 pb-20 md:pt-24 md:pb-28">
      <Image
        ...
        alt="Un père et son fils regardent un match de football en direct sur IRON TV"
        fill
        priority
        sizes="100vw"
      ...
      <div className="hero-scrim-desktop pointer-events-none absolute inset-0 hidden md:block" />
      {/* Soft glowing light blooms, screened on top for depth without flattening the photo */}
      <div className="hero-bloom animate-glow pointer-events-none absolute inset-0" />
      ...
      <div className="container-page relative mt-14 flex flex-col items-center text-center md:mt-20 md:block">
        <div className="md:flex md:max-w-[48%] md:flex-col md:items-start md:text-left lg:max-w-[44%]">
          <div className="animate-fade-up inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/5 px-4 py-1.5 text-[13px] font-medium text-white/80 backdrop-blur">
          ...
          <div
            className="animate-fade-up mt-14 flex flex-wrap items-center justify-center gap-x-10 gap-y-4 md:justify-start"
          ...
          >
            <div className="flex -space-x-2.5">
              {AVATAR_PHOTOS.map((src) => (
                <span
                  key={src}
                  className="h-9 w-9 shrink-0 overflow-hidden rounded-full border-2 border-ink"
                >
